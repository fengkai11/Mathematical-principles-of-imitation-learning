...
$$
a\sim p_\theta(a|o,z) \tag{G.13}$$

训练时 encoder 使用观测和专家动作：

$$
q_\phi(z|o,a) \tag{G.14}$$

decoder 根据 $o,z$ 重建动作：

$$
\hat a\sim p_\theta(a|o,z) \tag{G.15}$$
...
p_\theta(a|o)
=
\int p_\theta(a|o,z)p(z)dz \tag{G.17}$$

这个积分通常不好直接优化。

于是引入近似后验 $q_\phi(z|o,a)$，可以得到 ELBO：
...
$$
D_{\mathrm{KL}}(q_\phi(z|o,a)\|p(z)) \tag{G.20}$$
...
+
\beta
D_{\mathrm{KL}}(q_\phi(z|o,a)\|p(z)) \tag{G.21}$$
...
q_\phi(z|o,a)
=
\mathcal N(\mu_\phi(o,a),\sigma_\phi^2(o,a)I) \tag{G.22}$$
...
\quad
\epsilon\sim\mathcal N(0,I) \tag{G.23}$$
...
$$
A_t=(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{G.25}$$
...
\|A_t-\hat A_t\|^2]
+
\beta D_{\mathrm{KL}}(q_\phi(z|o,A_t)\|p(z)) \tag{G.26}$$
...
\quad
\epsilon\sim\mathcal N(0,I) \tag{G.30}$$
...
$$
p_\theta(A_t|o_t) \tag{G.33}$$

去噪网络写作：

$$
\epsilon_\theta(x_t,t,o_t) \tag{G.34}$$
