...
$$
\pi_\theta(a_t\mid x_t) \tag{13.1}$$

或者加上 latent：

$$
p_\theta(a_t\mid x_t,z) \tag{13.2}$$
...
$$
a_{t:t+H}=(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{13.3}$$

$$
p_\theta(a_{t:t+H}\mid x_t,z) \tag{13.4}$$

$$
q_\phi(z\mid x_t,a_{t:t+H}) \tag{13.5}$$
...
+
\beta
\underbrace{D_{\mathrm{KL}}(q_\phi(z\mid x_t,a_{t:t+H})\|p(z))}_{\text{latent 约束}} \tag{13.6}$$
...
\bar a_t
=
\frac{\sum_{i=0}^{K-1}w_i\hat a_t^{(t-i)}}{\sum_{i=0}^{K-1}w_i} \tag{13.7}$$
...
$$
\mathcal D=\{(x_t,a_t)\}_{t=1}^N \tag{13.8}$$

训练目标是让模型在看到 $x_t$ 时预测专家动作 $a_t$。如果是连续动作，可以写成：

$$
\hat a_t=f_\theta(x_t) \tag{13.9}$$

$$
\mathcal L_{\mathrm{step}}(\theta)
=
\frac{1}{N}\sum_{t=1}^{N}\|a_t-\hat a_t\|^2 \tag{13.10}$$
...
$$
a_{t:t+H}=(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{13.11}$$
...
$$
a_t,a_{t+1},\dots,a_{t+H-1} \tag{13.13}$$
...
$$
p_\theta(a\mid x,z) \tag{13.14}$$

ACT 把单个动作 $a$ 换成动作块 $a_{t:t+H}$：

$$
p_\theta(a_{t:t+H}\mid x_t,z) \tag{13.15}$$
...
$$
\hat a_{t:t+H}=f_\theta(x_t,z) \tag{13.16}$$
...
$$
q_\phi(z\mid x_t,a_{t:t+H}) \tag{13.18}$$

decoder 或 policy 写成：

$$
p_\theta(a_{t:t+H}\mid x_t,z) \tag{13.19}$$

这和第9章的 CVAE 结构完全对应，只是把 $a$ 换成了 $a_{t:t+H}$。
...
=
\mathcal N(z;\mu_\phi(x_t,a_{t:t+H}),\mathrm{diag}(\sigma_\phi^2(x_t,a_{t:t+H}))) \tag{13.20}$$
...
\quad
\epsilon\sim\mathcal N(0,I) \tag{13.21}$$
...
$$
h_t=\mathrm{Transformer}_\theta(\mathrm{tokens}(x_t),z) \tag{13.22}$$

再由输出头生成动作块：

$$
\hat a_{t:t+H}=g_\theta(h_t) \tag{13.23}$$
...
+
\beta\mathcal L_{\mathrm{KL}} \tag{13.24}$$
...
+
\beta
\underbrace{D_{\mathrm{KL}}(q_\phi(z\mid x_t,a_{t:t+H})\|p(z))}_{\text{latent 约束}} \tag{13.25}$$
...
\mathcal L_{\mathrm{chunk}}
=
\sum_{j=0}^{H-1}\|a_{t+j}-\hat a_{t+j}\|^2 \tag{13.26}$$
...
\mathcal L_{\mathrm{chunk}}
=
\frac{1}{H}\sum_{j=0}^{H-1}\|a_{t+j}-\hat a_{t+j}\|^2 \tag{13.27}$$
...
=
D_{\mathrm{KL}}(q_\phi(z\mid x_t,a_{t:t+H})\|p(z)) \tag{13.28}$$
...
=
\mathcal L_{\mathrm{chunk}}+
\beta\mathcal L_{\mathrm{KL}} \tag{13.29}$$
...
\bar a_t
=
\frac{\sum_{i=0}^{K-1}w_i\hat a_t^{(t-i)}}{\sum_{i=0}^{K-1}w_i} \tag{13.31}$$

常见权重可以让新预测权重大、旧预测权重小，比如：

$$
w_i=\exp(-\lambda i) \tag{13.32}$$
...
$$
\hat a_{t:t+H}=f_\theta(x_t,z) \tag{13.33}$$

系统只执行前 $k$ 步：

$$
\hat a_{t:t+k} \tag{13.34}$$

到 $t+k$ 时刻，拿到新观测 $x_{t+k}$，再预测：

$$
\hat a_{t+k:t+k+H}=f_\theta(x_{t+k},z') \tag{13.35}$$
...
$$
q_\phi(z\mid x,a) \tag{13.36}$$

$$
p_\theta(a\mid x,z) \tag{13.37}$$

ACT 的对应关系是：

$$
q_\phi(z\mid x_t,a_{t:t+H}) \tag{13.38}$$

$$
p_\theta(a_{t:t+H}\mid x_t,z) \tag{13.39}$$
...
$$
\hat u_{t:t+H}=f_\theta(x_t) \tag{13.41}$$

或者输出未来轨迹点：

$$
\hat y_{t:t+H}=f_\theta(x_t) \tag{13.42}$$
...
$$
\text{approach} \rightarrow \text{align} \rightarrow \text{insert} \rightarrow \text{release} \tag{13.43}$$
...
$$
\tau=(x_0,a_0,x_1,a_1,\dots,x_T,a_T) \tag{13.44}$$

我们可以从中构造训练样本：

$$
(x_t,a_{t:t+H}) \tag{13.45}$$
...
=
\{(x_t,a_{t:t+H})\}_{t=0}^{T-H} \tag{13.46}$$
...
$$
\frac1H\sum_{j=0}^{H-1}\|a_{t+j}-\hat a_{t+j}\|^2 \tag{13.47}$$

也可以分不同 horizon 看误差：

$$
\ell_j=\|a_{t+j}-\hat a_{t+j}\|^2 \tag{13.48}$$
...
$$
a_{t:t+H}=(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{13.51}$$

ACT 可以写成一个条件动作块生成模型：

$$
p_\theta(a_{t:t+H}\mid x_t,z) \tag{13.52}$$

训练时使用 CVAE encoder：

$$
q_\phi(z\mid x_t,a_{t:t+H}) \tag{13.53}$$
...
\mathcal L_{\mathrm{chunk}}
+
\beta D_{\mathrm{KL}}(q_\phi(z\mid x_t,a_{t:t+H})\|p(z)) \tag{13.54}$$
...
\bar a_t
=
\frac{\sum_{i=0}^{K-1}w_i\hat a_t^{(t-i)}}{\sum_{i=0}^{K-1}w_i} \tag{13.55}$$
...
7. $\beta$ 太大和太小分别可能导致什么问题？请结合第9章的 posterior collapse 和 prior mismatch 解释。
