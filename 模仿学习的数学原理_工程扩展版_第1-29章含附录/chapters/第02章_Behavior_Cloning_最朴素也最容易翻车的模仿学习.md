...
因此，本章不仅要说明 BC “像监督学习”，还要证明两件事：

1. 为什么概率 BC 可以写成负对数似然损失；
2. 为什么常用的 MSE 可以看成固定方差高斯假设下的 NLL 特例。
...
1. BC 为什么可以被写成监督学习；
2. 概率策略为什么自然对应最大似然和 NLL；
3. MSE 为什么可以看成固定方差高斯策略的 NLL 特例；
4. MSE 隐含的是单峰固定方差假设，面对多模态动作会有风险；
5. BC 优化的是专家访问分布上的单步损失，不等于闭环执行目标；
6. 验证集 loss 很低，只能说明模型在专家数据分布上会答题，不说明实机闭环一定成功。
...
> **BC 是专家分布上的单步监督学习；最大似然可以写成 NLL；MSE 是固定方差高斯策略下的 NLL 特例；这三点共同说明 BC 简单有效，但也为分布偏移和多模态动作问题埋下伏笔。**
...
2. 为什么最大化专家动作概率等价于最小化 NLL？
3. 固定方差高斯动作模型为什么会推出 MSE？
4. MSE 隐含了什么动作分布假设？为什么多模态动作下可能出问题？
5. 为什么 BC 的专家分布目标不能保证学习策略闭环成功？
6. 在机械臂抓取任务中，你会如何发现训练 loss 低但闭环失败的问题？
