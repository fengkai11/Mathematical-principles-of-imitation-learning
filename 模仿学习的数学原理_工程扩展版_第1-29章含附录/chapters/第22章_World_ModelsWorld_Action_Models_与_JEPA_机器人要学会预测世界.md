...
第21章提出从动作模仿走向动作后果预测：机器人不能只会背动作表。本章给出数学接口：从 MDP 转移概率到 latent dynamics，从像素重建到表示预测，从普通 world model 到 action-conditioned world action model。

第23章会继续讨论：世界预测学到之后，真实机器人系统如何使用它。也就是如何把预测结果用于候选动作评估、短期 rollout、风险预测、失败恢复和安全边界。
...
由于真实状态 $s_t$ 难以获得，我们通常先把观测历史编码成隐表示，即公式 (22.3)：

$$h_t=f_\phi(o_{\le t})$$

再学习 latent dynamics，即公式 (22.4)：

$$p_\eta(h_{t+1} \mid h_t,a_t)$$

如果使用确定性预测器，可以写成公式 (22.5)：

$$\hat h_{t+1}=F_\eta(h_t,a_t)$$

如果要预测多步未来，可以写成：

**公式 (22.8)：多步 latent rollout**
...
为了避免误解，可以把三种预测方式放在一起看：

| 方式 | 预测目标 | 优点 | 风险 |
|---|---|---|---|
| 像素预测 | 未来图像 | 直观，可视化强 | 容易浪费容量在纹理和背景 |
| 状态预测 | 物体位姿、速度、接触 | 工程含义清楚 | 需要标注或状态估计 |
| 表示预测 | latent state | 可自监督，适合大规模数据 | 表示未必包含控制关键变量 |
...
- $r_\eta$：任务相关预测头；
- $y_{i,t+1}$：真实任务标签，例如是否碰撞、是否成功、是否接管；
- $\ell$：分类、回归或排序损失。
...
不要把“预测 loss 低”直接等同于“机器人更安全”。对机器人来说，错过一个关键风险，比预测错很多背景纹理更严重。
...
8. 第23章要继续回答：世界预测学到之后，真实机器人系统如何使用它。
...
| 编号 | 概念 | 一句话含义 |
|---|---|---|
| 定义 22.1 | World Model | 预测世界状态、表示或任务变量如何随时间变化的模型 |
| 定义 22.2 | Latent dynamics | 在隐空间中预测未来状态或表示的动力学模型 |
| 定义 22.3 | World Action Model | 显式输入动作或动作块，预测动作后果的世界模型 |
| 定义 22.4 | JEPA 式表示预测 | 不重建像素，而是在隐空间预测未来表示的预测思想 |
...
- **图22-1 VLA、World Model、WAM 与 JEPA 关系图**：说明动作策略、世界预测、动作条件预测和表示预测的区别。
...
3. **Yann LeCun et al., JEPA / I-JEPA / V-JEPA 系列工作**
   - 类型：前沿 / 表示预测。
   - 阅读目的：理解不重建像素、预测表示的自监督路线。
   - 重点看什么：重点看预测目标和 representation collapse 的处理方式。
   - 对应本章：公式 (22.7) 的思想背景。

4. **机器人 model-based planning 相关论文**
   - 类型：工程 / 规划与控制。
   - 阅读目的：理解如何用模型 rollout、评估候选动作和规避风险。
   - 重点看什么：重点看预测 horizon、误差累积和安全约束。
   - 对应本章：世界预测如何服务第23章的策略系统。
...
本章最重要的警惕是命题 22.1：预测误差小不必然推出控制效果好。因此，世界模型必须和任务关键变量、安全阈值、接触状态、失败恢复和实机验证绑定起来。

第23章将继续回答：世界预测学到之后，如何真正进入机器人策略系统，服务候选动作评估、短期 rollout、风险预测、失败恢复和安全边界。
