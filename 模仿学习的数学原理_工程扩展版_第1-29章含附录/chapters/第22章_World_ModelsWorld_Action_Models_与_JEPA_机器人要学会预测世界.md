...
## 2. 为什么只学 $\pi(a\mid o)$ 不够？

策略 $\pi(a\mid o)$ 关注的是当前观测到动作的映射。它回答：
...
**符号**：$s_t$ 是真实环境状态；$a_t$ 是动作；$P(s_{t+1}\mid s_t,a_t)$ 是给定当前状态和动作后，下一状态的概率分布。

**直觉**：如果我推杯子，杯子会移动；如果我夹得太偏，杯子会倾斜；如果我撞到治具边缘，动作会失败。

**工程含义**：真实 $s_t$ 通常不可完全观测，所以工程上不会直接学完整物理状态转移，而是学观测或隐表示的转移。
...
**符号**：$f_\phi$ 是编码器；$z_t$ 是观测隐表示；$p_\theta$ 是隐空间动力学模型；$H$ 是预测 horizon。
...
其中 $c_t$ 可以包含语言指令、任务阶段、机器人本体状态或对象关系图。如果预测的是一段未来：
...
**符号**：$F_\theta$ 是动作条件的世界预测模型；$c_t$ 是上下文；$A_t$ 是候选动作块。
...
**符号**：$g_\theta$ 是预测器；$z_t$ 和 $z_{t+1}$ 是前后时刻的隐表示；$\mathrm{sg}$ 用来稳定目标表示。
...
这里 $\mathcal{R}_{\mathrm{repr}}$ 用来保证表示不要全部变成常数。
...
- **附录A：数学符号与公式阅读方法**：理解 $s,z,o,a$ 的区别。
