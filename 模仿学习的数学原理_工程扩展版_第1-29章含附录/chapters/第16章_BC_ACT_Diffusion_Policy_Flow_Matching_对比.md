...
$$
A_t \sim \pi_\theta(A_t \mid o_t) \tag{16.1}$$
...
| ACT | 动作块 $A_t$ | Transformer + action chunk | 需要局部时间结构的操作任务 |
...
a_{t:t+H-1}
=
(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{16.9}$$

如果每个动作维度为 $d_a$，动作块可以看成：

$$
A_t\in\mathbb R^{H\times d_a} \tag{16.10}$$

ACT 希望学习的是：

$$
p_\theta(A_t|o_{\le t},z) \tag{16.11}$$
...
\quad
\epsilon\sim\mathcal N(0,I) \tag{16.17}$$
...
$$
\epsilon_\theta(A^{(k)},k,obs_t) \tag{16.18}$$
...
$$
\text{model input} \longrightarrow \text{action object} \tag{16.21}$$
...
$$
o_t \longrightarrow a_t \tag{16.22}$$

ACT：

$$
o_{\le t} \longrightarrow A_t=a_{t:t+H-1} \tag{16.23}$$
...
$$
\hat a=f_\theta(o) \tag{16.25}$$

条件分布写作：

$$
a\sim p_\theta(a|o) \tag{16.26}$$

动作块条件分布写作：

$$
A\sim p_\theta(A|obs) \tag{16.27}$$
...
+
\beta D_{\mathrm{KL}}
\left(q_\phi(z|o,A)\|p(z)\right) \tag{16.29}$$
...
$$
H_{\mathrm{BC}}=1 \tag{16.31}$$

ACT 和 Diffusion Policy 通常生成长度为 $H$ 的动作块：

$$
A_t=a_{t:t+H-1} \tag{16.32}$$
...
$$
C_{\mathrm{BC}}\approx C_f \tag{16.35}$$

ACT 通常也是一次前向，但输出更长序列：

$$
C_{\mathrm{ACT}}\approx C_g \tag{16.36}$$
...
$$
A_t=(a_t,a_{t+1},\dots,a_{t+H-1}) \tag{16.39}$$
...
$$
A\sim p_\theta(A|obs) \tag{16.40}$$

而不是只学习：

$$
\hat A=f_\theta(obs) \tag{16.41}$$
...
当 $p(a|o)$ 有多个峰时，MSE 点估计可能落在峰之间。这个问题在第7章已经讲过，本章再次强调，因为它是从 BC 升级到生成式策略的核心动机之一。
...
$$
p_\theta(A_t|o_{\le t},z) \tag{16.48}$$
...
+
\beta D_{\mathrm{KL}}
\left(q_\phi(z|o,A)\|p(z)\right) \tag{16.49}$$
...
\sqrt{\bar\alpha_k}A^{(0)}
+
\sqrt{1-\bar\alpha_k}\,\epsilon \tag{16.50}$$
...
\quad
C_{\mathrm{diffusion}}\approx K C_\epsilon \tag{16.53}$$
