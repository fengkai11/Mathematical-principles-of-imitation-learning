...
Transformer 的核心操作是 self-attention。给定长度为 $T$、隐藏维度为 $d$ 的序列，它大致需要构造 $T\times T$ 的注意力关系：
...
**动机**：机器人长时域控制中，历史长度 $T$ 很容易变大。比如 50Hz 控制下保留 20 秒历史，就是 1000 个时间步。

**符号**：$T$ 是序列长度；$d$ 是每个 token 的隐藏维度；$\mathcal{O}(T^2d)$ 表示主要计算量随 $T^2$ 增长。
...
状态空间模型 SSM 最容易从连续时间系统理解。假设输入是 $u(t)$，隐藏状态是 $h(t)$，输出是 $y(t)$：
...
**动机**：我们希望模型用一个隐藏状态 $h(t)$ 记住历史，而不是每次都把所有历史 token 重新拿出来做两两 attention。

**符号**：$A$ 控制隐藏状态自身如何演化；$B$ 控制输入如何写入状态；$C$ 控制如何从状态读出输出；$D$ 是输入到输出的直接通道。

**公式**：第一部分是状态方程，第二部分是输出方程。

**直觉**：SSM 像一个带记忆的滤波器。输入不断进来，隐藏状态不断更新，输出从当前状态读出。

**工程含义**：机器人每来一帧观测，就更新一次 hidden state。历史信息被压缩在 $h(t)$ 里，不需要把所有过去观测都重新参与计算。
...
**动机**：真实系统每隔 $\Delta$ 秒更新一次，连续微分方程必须变成离散递推，才能进工程代码。

**符号**：$k$ 是离散时间步；$\bar{A},\bar{B},\bar{C},\bar{D}$ 是离散化后的参数；$\Delta$ 是时间间隔。

**直觉**：连续系统像水流，离散系统像每隔一小段时间拍一张照片。离散化就是把“连续流动”变成“逐帧更新”。

**工程含义**：如果控制频率变化，$\Delta$ 变化，状态更新也应该考虑频率影响。实际神经网络版本会把这些矩阵参数化、学习化，但这个动力系统直觉非常重要。
...
这个公式说明，当前输出 $y_k$ 仍然受所有历史输入影响，只是影响通过 $\bar{A}^{k-i}$ 被压缩和衰减。
...
**符号**：$h_k$ 是到当前时刻为止的历史摘要；$u_k$ 是当前输入 token；$y_k$ 是输出表示。

**直觉**：hidden state 像一个滚动笔记本。每次新观测来了，模型更新笔记；动作头只看当前笔记和当前观测。

**工程含义**：机械臂末端运动中，$h_k$ 可以记住“夹爪刚接触物体”“上一阶段发生过滑动”“目标已经接近但姿态还没对齐”。这些信息仅靠当前图像未必能看出来。
...
**符号**：$\mathrm{Select}_\theta$ 表示由当前输入决定 SSM 参数的选择机制；下标 $k$ 表示这些参数随时间变化。
...
这里 $z_k$ 是序列骨架输出，$p_\psi$ 可以是 ACT、Diffusion Policy 或 Flow Matching 动作头。
...
其中 $g_k$ 是慢模型给出的子目标或阶段指令。这个公式的工程含义是：快模型把上一个隐藏状态、当前观测和高层目标结合起来，输出动作块，并把新状态缓存到下一周期。
...
**工程含义**：Mamba 降低的是 $T_{\mathrm{seq}}$ 中长序列建模的部分，但图像编码、动作生成头和安全仲裁仍然可能成为瓶颈。
