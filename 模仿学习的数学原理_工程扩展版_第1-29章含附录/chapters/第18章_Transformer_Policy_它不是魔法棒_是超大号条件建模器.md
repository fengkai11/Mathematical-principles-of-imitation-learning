...
$$
P_\theta(a_t|o_{\le t},a_{<t},g) \tag{18.1}$$
...
$$
P_\theta(a_t|o_{\le t},a_{<t},g) \tag{18.3}$$
...
$$
P_\theta(a_t|o_{\le t},a_{<t},g) \tag{18.4}$$
...
x_i
=
\mathrm{Embed}(u_i)+p_i+c_i \tag{18.6}$$
...
x_i
=
\mathrm{Embed}(u_i)+p_i+c_i \tag{18.7}$$
...
$$
Q=XW_Q,\quad K=XW_K,\quad V=XW_V \tag{18.8}$$
...
=
\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}\right)V \tag{18.9}$$
...
\mathrm{Attention}(Q,K,V)
=
\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}\right)V \tag{18.10}$$
...
\mathrm{Attention}(Q,K,V)
=
\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}+M\right)V \tag{18.12}$$
...
h_t
=
(o_{t-H+1:t},a_{t-H+1:t-1}) \tag{18.14}$$
...
P_\theta(a_t|h_t,g)
=
P_\theta(a_t|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.15}$$
...
h_t
=
(o_{t-H+1:t},a_{t-H+1:t-1}) \tag{18.16}$$
...
N_{\mathrm{token}}
\approx
H\cdot(N_{\mathrm{vision}}+N_{\mathrm{state}}+N_{\mathrm{action}})+N_{\mathrm{goal}} \tag{18.17}$$

这条式子提醒我们：多摄像头、高分辨率、长历史叠在一起，token 数会涨得很快。attention 的计算量通常和 token 数的平方相关：

$$
\mathcal O(N_{\mathrm{token}}^2) \tag{18.18}$$
...
$$
P_\theta(a_{t:t+K-1}|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.21}$$
...
$$
\pi_\theta(a_t|o_t) \tag{18.22}$$

Transformer policy 常写成：

$$
P_\theta(a_t|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.23}$$

或动作块形式：

$$
P_\theta(a_{t:t+K-1}|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.24}$$
...
\hat a_t
=
f_\theta(o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.26}$$
...
$$
\hat a_t=f_\theta(\mathrm{context}_t) \tag{18.29}$$
...
$$
a_t \rightarrow y_t \in \{1,2,\dots,V\} \tag{18.30}$$

其中 $V$ 是动作词表大小。模型预测：

$$
P_\theta(y_t|\mathrm{context}_t) \tag{18.31}$$
...
$$
a_t \rightarrow y_t \in \{1,2,\dots,V\} \tag{18.32}$$
...
$$
P_\theta(a_t|I_{\le t}^{1:m},q_{\le t},a_{<t},l) \tag{18.33}$$
...
$$
P_\theta(a_t|I_{\le t}^{1:m},q_{\le t},a_{<t},l) \tag{18.34}$$
...
$$
\mathcal D
\text{ 没有覆盖的状态—动作区域，模型仍然缺少可靠监督。} \tag{18.35}$$
...
$$
\mathcal D=\{\tau_i\}_{i=1}^{N} \tag{18.37}$$

每条轨迹包含：

$$
\tau_i=(o_0,a_0,o_1,a_1,\dots,o_T,a_T,g) \tag{18.38}$$
...
$$
(o_{t-H+1:t},a_{t-H+1:t-1},g) \rightarrow a_t \tag{18.39}$$

或动作块形式：

$$
(o_{t-H+1:t},a_{t-H+1:t-1},g) \rightarrow a_{t:t+K-1} \tag{18.40}$$
...
$$
P_\theta(u_t|z_{t-H+1:t},u_{t-H+1:t-1},g_{\mathrm{slot}}) \tag{18.41}$$
...
$$
P_\theta(a_t|o_{\le t},a_{<t},g) \tag{18.43}$$
...
$$
x_{1:n}=(x_1,x_2,\dots,x_n) \tag{18.44}$$
...
$$
x_i=\mathrm{Embed}(u_i)+p_i+c_i \tag{18.45}$$
...
$$
Q=XW_Q,\quad K=XW_K,\quad V=XW_V \tag{18.46}$$
...
\mathrm{Attention}(Q,K,V)
=
\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}\right)V \tag{18.47}$$
...
\mathrm{Attention}(Q,K,V)
=
\mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d_k}}+M\right)V \tag{18.49}$$
...
$$
h_t=(o_{t-H+1:t},a_{t-H+1:t-1}) \tag{18.50}$$
...
P_\theta(a_t|h_t,g)
=
P_\theta(a_t|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.51}$$
...
N_{\mathrm{token}}
\approx
H\cdot(N_{\mathrm{vision}}+N_{\mathrm{state}}+N_{\mathrm{action}})+N_{\mathrm{goal}} \tag{18.52}$$
...
$$
\mathcal O(N_{\mathrm{token}}^2) \tag{18.53}$$
...
$$
P_\theta(a_{t:t+K-1}|o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.54}$$
...
$$
\hat a_t=f_\theta(o_{t-H+1:t},a_{t-H+1:t-1},g) \tag{18.56}$$
...
$$
a_t \rightarrow y_t\in\{1,2,\dots,V\} \tag{18.58}$$
...
$$
P_\theta(a_t|I_{\le t}^{1:m},q_{\le t},a_{<t},l) \tag{18.59}$$
...
$$
P_\theta(u_t|z_{t-H+1:t},u_{t-H+1:t-1},g_{\mathrm{slot}}) \tag{18.60}$$
