...
## 6. 动作表示：连续动作、动作 token 与动作块

VLA 可以使用不同动作头。这里不把每一种都写成全书核心公式，而是保留三种最重要的接口。
...
**公式 (18.5)：动作 token 条件分布**

$$p_\theta(y_t\mid c_t^{VLA})$$
...
## 7. 命题 18.1：VLA 仍然是多模态上下文上的条件 BC
...
连续动作时，公式 (18.7) 可以落到第2章和第5章已经讨论过的高斯 NLL 或 MSE；动作 token 时，它可以落到交叉熵；动作块或生成式动作头时，它可以落到第4章和第四篇已经介绍过的动作块分布学习。

所以，VLA 的数学底座并没有脱离模仿学习。它的升级点是条件变量更丰富，动作表示更灵活。
...
## 10. Policy = Context Encoder + Action Decoder
...
4. 动作应该用连续向量、动作 token、动作块，还是生成式动作头表示。
5. 训练上下文和部署上下文是否一致。
6. 当前失败更应该升级 Context Encoder，还是升级 Action Decoder。
7. VLA 的收益是否足以抵消数据、延迟、安全和调试成本。
...
- **图18-1 VLA 输入输出结构**：展示视觉、语言、本体状态、历史动作进入多模态上下文，并输出连续动作、动作 token 或动作块。
- **图18-2 Context Encoder 与 Action Decoder 拆解**：展示上下文建模和动作生成是两个可组合模块。
- **图18-3 语言-视觉-动作 grounding**：展示语言对象、图像区域、空间关系和动作序列之间的对齐关系。
- **图18-4 表达能力与工程风险**：展示模型能力增强不必然降低部署风险。
- **图18-5 VLA 与第六篇世界预测的衔接**：展示 VLA 输出动作后，仍需要预测动作后果与安全边界。

---

## 21. 推荐阅读：重点看什么

- **RT-1 / RT-2 系列工作**：重点看如何把视觉、语言和动作组织成可训练的机器人策略，而不是只看模型规模。
- **OpenVLA / Octo / π0 等通用机器人策略工作**：重点看动作表示、数据混合、跨机器人泛化和部署边界。
