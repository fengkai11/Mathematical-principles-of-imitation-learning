...
第一类是**方向关系、符号声明、单行定义**，例如“给定 reward 求策略”这种关系。它们如果强行放成独立块级公式，页面上容易显示成很窄的细长条，影响阅读。因此本章统一改成：

```text
公式编号 + 行内公式 + 文字读法
```

第二类是**真正需要展开的推导式**，例如 feature expectation 推导、最大似然推导、MaxEnt IRL 梯度推导。这类公式仍然保留独立公式块，便于读者逐行看清推导。
...
由配分函数定义可得 **公式 (10.31)**：$Z(w)=\sum_{\tau}\exp(w^T F(\tau))$。
...
> 说明：本章中较短的方向关系、符号定义和单行定义，采用“公式编号 + 行内公式 + 文字读法”的方式呈现；真正需要读者逐行理解的推导，仍使用独立公式块。
