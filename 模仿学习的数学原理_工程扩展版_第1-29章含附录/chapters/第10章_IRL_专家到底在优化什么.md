...
本章先前使用过独立块级公式，但在当前页面中会出现逐字符竖排的问题，例如 $J(\pi)$ 被拆成多行。为保证 mdBook 和 GitHub 页面都可读，本章统一采用下面格式：

```text
公式编号 + 行内公式 + 文字读法
```

因此，本章不再使用 `$$...$$` 独立公式块。较长推导改成“第一步、第二步、第三步”的文字推导，每一步仍保留行内公式和公式编号。
...
**公式 (10.1)：普通 RL 的方向**：给定 reward $r(s,a)$，求一个能最大化回报的策略 $\pi^*$。

它的目标通常写成 **公式 (10.2)：策略的期望回报目标**：$J(\pi)=\mathbb{E}_{\tau \sim p_\pi(\tau)}[\sum_{t=0}^{T}\gamma^t r(s_t,a_t)]$。

这个公式可以这样读：策略 $\pi$ 和环境闭环交互会产生轨迹 $\tau$；每条轨迹上有一串奖励 $r(s_t,a_t)$；把这些奖励按折扣因子 $\gamma^t$ 加起来，再对所有可能轨迹求平均，就得到策略的期望回报 $J(\pi)$。
...
在这个设定下，最优策略定义为 **公式 (10.3)**：$\pi^*=\arg\max_\pi J(\pi)$。

IRL 的方向反过来。我们手里没有 reward，只有专家轨迹数据。

**公式 (10.4)：专家轨迹数据集**：$\mathcal{D}_E=\{\tau_i^E\}_{i=1}^{N}$。

**公式 (10.5)：专家轨迹形式**：$\tau_i^E=(s_0^i,a_0^i,s_1^i,a_1^i,\dots,s_T^i,a_T^i)$。

IRL 想学习一个 reward。

**公式 (10.6)：IRL 的方向**：给定专家数据 $\mathcal{D}_E$，反推一个能解释专家行为的 reward $r(s,a)$。
...
**公式 (10.7)：reward 缩放**：$r'(s,a)=\alpha r(s,a)$，其中 $\alpha>0$。

对于任意一条轨迹 $\tau$，原 reward 下的回报是 **公式 (10.8)**：$R(\tau)=\sum_{t=0}^{T}\gamma^t r(s_t,a_t)$。

新 reward 下的回报为 **公式 (10.9)**：$R'(\tau)=\sum_{t=0}^{T}\gamma^t r'(s_t,a_t)=\alpha\sum_{t=0}^{T}\gamma^t r(s_t,a_t)=\alpha R(\tau)$。

因为 $\alpha>0$，所以如果轨迹 $\tau_1$ 的回报大于轨迹 $\tau_2$，那么缩放后仍然有 **公式 (10.10)**：$R(\tau_1)>R(\tau_2) \Rightarrow R'(\tau_1)>R'(\tau_2)$。

轨迹排序没有变，最优轨迹集合也不会因为正比例缩放而改变。

再考虑固定长度任务中的常数平移。

**公式 (10.11)：reward 平移**：$r'(s,a)=r(s,a)+c$。

如果每条轨迹长度都是 $T+1$，且暂时忽略折扣差异，那么每条轨迹都会多出相同的常数项。更一般地，在固定 horizon 且折扣因子相同的情况下，多出的项与轨迹内容无关。

**公式 (10.12)：平移后的轨迹回报**：$R'(\tau)=R(\tau)+c\sum_{t=0}^{T}\gamma^t$。
...
**公式 (10.13)：线性 reward**：$r_w(s,a)=w^T f(s,a)$。
...
如果单步 reward 是线性特征组合，那么一条轨迹的累计特征可以写成 **公式 (10.14)**：$F(\tau)=\sum_{t=0}^{T}\gamma^t f(s_t,a_t)$。

对应的轨迹回报为 **公式 (10.15)**：$R_w(\tau)=\sum_{t=0}^{T}\gamma^t r_w(s_t,a_t)$。

把 $r_w(s,a)=w^T f(s,a)$ 代入后，可以分三步读 **公式 (10.16)**：第一步，$R_w(\tau)=\sum_{t=0}^{T}\gamma^t w^T f(s_t,a_t)$；第二步，把不随时间变化的 $w^T$ 提出来，得到 $R_w(\tau)=w^T\sum_{t=0}^{T}\gamma^t f(s_t,a_t)$；第三步，根据 $F(\tau)$ 的定义，得到 $R_w(\tau)=w^T F(\tau)$。
...
定义策略 $\pi$ 的特征期望为 **公式 (10.17)**：$\mu(\pi)=\mathbb{E}_{\tau \sim p_\pi(\tau)}[F(\tau)]$。

策略的期望回报可以按 **公式 (10.18)** 分四步读：第一步，$J_w(\pi)=\mathbb{E}_{\tau \sim p_\pi(\tau)}[R_w(\tau)]$；第二步，代入 $R_w(\tau)=w^T F(\tau)$，得到 $J_w(\pi)=\mathbb{E}_{\tau \sim p_\pi(\tau)}[w^T F(\tau)]$；第三步，把 $w^T$ 提到期望外，得到 $J_w(\pi)=w^T\mathbb{E}_{\tau \sim p_\pi(\tau)}[F(\tau)]$；第四步，代入特征期望定义，得到 $J_w(\pi)=w^T\mu(\pi)$。
...
**公式 (10.19)：最大熵轨迹分布**：$p_w(\tau)=\frac{1}{Z(w)}\exp(R_w(\tau))$。

其中配分函数为 **公式 (10.20)**：$Z(w)=\sum_{\tau}\exp(R_w(\tau))$。
...
专家数据为 **公式 (10.21)**：$\mathcal{D}_E=\{\tau_i^E\}_{i=1}^{N}$。

最大似然目标是让专家轨迹在模型 $p_w(\tau)$ 下概率尽量大。

**公式 (10.22)：专家轨迹 log likelihood**：$\mathcal{L}(w)=\sum_{i=1}^{N}\log p_w(\tau_i^E)$。

把 $p_w(\tau)$ 代入后，**公式 (10.23)** 可以分两步读：第一步，$\mathcal{L}(w)=\sum_{i=1}^{N}\log \frac{1}{Z(w)}\exp(R_w(\tau_i^E))$；第二步，利用 $\log \exp(x)=x$ 和 $\log \frac{1}{Z(w)}=-\log Z(w)$，得到 $\mathcal{L}(w)=\sum_{i=1}^{N}R_w(\tau_i^E)-N\log Z(w)$。
...
**公式 (10.24)：专家特征期望**：$\mu_E=\frac{1}{N}\sum_{i=1}^{N}F(\tau_i^E)$。

**公式 (10.25)：模型特征期望**：$\mu_w=\mathbb{E}_{\tau \sim p_w(\tau)}[F(\tau)]$。

**公式 (10.26)：平均 log likelihood**：$\bar{\mathcal{L}}(w)=\frac{1}{N}\mathcal{L}(w)$。
...
由式 (10.23) 得到 **公式 (10.28)**：$\bar{\mathcal{L}}(w)=\frac{1}{N}\sum_{i=1}^{N}R_w(\tau_i^E)-\log Z(w)$。

因为 $R_w(\tau)=w^T F(\tau)$，所以 **公式 (10.29)**：$\nabla_w R_w(\tau)=F(\tau)$。

第一项的梯度为 **公式 (10.30)**：$\nabla_w \frac{1}{N}\sum_{i=1}^{N}R_w(\tau_i^E)=\frac{1}{N}\sum_{i=1}^{N}F(\tau_i^E)=\mu_E$。

接下来处理 $\log Z(w)$。由配分函数定义可得 **公式 (10.31)**：$Z(w)=\sum_{\tau}\exp(w^T F(\tau))$。

对 $Z(w)$ 求梯度，得到 **公式 (10.32)**：$\nabla_w Z(w)=\sum_{\tau}\exp(w^T F(\tau))F(\tau)$。

因此，**公式 (10.33)** 可以分六步读：第一步，$\nabla_w \log Z(w)=\frac{1}{Z(w)}\nabla_w Z(w)$；第二步，代入 $\nabla_w Z(w)$，得到 $\nabla_w \log Z(w)=\frac{1}{Z(w)}\sum_{\tau}\exp(w^T F(\tau))F(\tau)$；第三步，把 $1/Z(w)$ 放进求和，得到 $\nabla_w \log Z(w)=\sum_{\tau}\frac{\exp(w^T F(\tau))}{Z(w)}F(\tau)$；第四步，根据 $p_w(\tau)=\frac{1}{Z(w)}\exp(w^T F(\tau))$，得到 $\nabla_w \log Z(w)=\sum_{\tau}p_w(\tau)F(\tau)$；第五步，把求和写成期望，得到 $\nabla_w \log Z(w)=\mathbb{E}_{\tau \sim p_w(\tau)}[F(\tau)]$；第六步，代入模型特征期望定义，得到 $\nabla_w \log Z(w)=\mu_w$。
...
**公式 (10.35)：occupancy measure 记号**：$\rho_\pi(s,a)$ 表示策略 $\pi$ 在闭环执行时，有多经常访问状态 $s$ 并执行动作 $a$。
...
> 说明：为了避免当前页面把块级公式渲染成逐字符竖排，本章所有公式都采用“公式编号 + 行内公式 + 文字读法”的方式呈现，不再使用独立块级公式。
