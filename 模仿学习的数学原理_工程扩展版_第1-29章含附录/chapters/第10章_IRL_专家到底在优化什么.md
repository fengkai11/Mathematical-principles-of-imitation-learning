...
本章会在前面“状态、动作、策略、轨迹分布”的基础上，引入一组用于描述专家偏好的数学对象。读者先不需要一次性记住所有符号，但需要理解它们在主线中的作用。

| 数学对象 | 符号 | 一句话含义 | 工程对应物 | 后续作用 |
|---|---|---|---|---|
| 奖励函数 | $r(s,a)$ | 描述状态—动作好坏的标量函数 | 安全、平顺、成功率、碰撞风险等偏好 | IRL、RL、后训练 |
| 轨迹回报 | $R(\tau)$ | 一条轨迹累计得到的 reward | 一次抓取、放置或泊车全过程质量 | 连接轨迹级目标 |
...
这些对象共同完成一个转变：从“专家在某个状态下做了什么动作”，转向“专家在整条轨迹上体现了什么偏好”。
...
- 附录 C：最大似然、负对数似然、交叉熵与 KL 散度。建议结合本章第8节理解最大似然目标。
