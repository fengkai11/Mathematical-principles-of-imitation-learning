...
$$
\pi_E \;\text{或}\; \mathcal D_E \longrightarrow r(s,a) \tag{10.2}$$
...
$$
\pi_\theta(a|s) \approx \pi_E(a|s) \tag{10.3}$$

IRL 关注：

$$
r_\phi(s,a) \;\text{能不能解释专家为什么偏好某些行为？} \tag{10.4}$$
...
$$
(\mathcal S,\mathcal A,P,R,\gamma) \tag{10.5}$$
...
\tau_i^E
=
(s_0^i,a_0^i,s_1^i,a_1^i,\dots,s_T^i,a_T^i) \tag{10.9}$$
...
$$
r'(s,a)=\alpha r(s,a),\quad \alpha>0 \tag{10.13}$$
...
$$
r'(s,a)=\alpha r(s,a),\quad \alpha>0 \tag{10.15}$$
...
r_w(s,a)
=
\sum_{k=1}^{K} w_k f_k(s,a) \tag{10.20}$$
...
R_w(\tau)
=
\sum_{t=0}^{T}\gamma^t r_w(s_t,a_t) \tag{10.22}$$
...
R_w(\tau)
=
\sum_{t=0}^{T}\gamma^t w^\top f(s_t,a_t) \tag{10.23}$$
...
F(\tau)
=
\sum_{t=0}^{T}\gamma^t f(s_t,a_t) \tag{10.25}$$

于是：

$$
R_w(\tau)=w^\top F(\tau) \tag{10.26}$$
...
$$
F(\tau)=\sum_{t=0}^{T}\gamma^t f(s_t,a_t) \tag{10.27}$$
...
\sum_{i=1}^{N}
\sum_{t=0}^{T}
\gamma^t f(s_t^i,a_t^i) \tag{10.29}$$
...
$$
\mu(\pi)\approx\mu_E \tag{10.34}$$

那么对任意线性 reward 权重 $w$，它和专家的期望回报都会接近：

$$
w^\top\mu(\pi)\approx w^\top\mu_E \tag{10.35}$$
...
\mu(\pi)
\approx
\sum_{s,a}\rho_\pi(s,a) f(s,a) \tag{10.36}$$
...
\sum_{i=1}^{N}
\sum_{t=0}^{T}
\gamma^t f(s_t^i,a_t^i) \tag{10.37}$$
...
p_w(\tau)
=
\frac{1}{Z(w)}\exp(R_w(\tau)) \tag{10.38}$$
...
p_w(\tau)
=
\frac{1}{Z(w)}\exp(R_w(\tau)) \tag{10.39}$$

其中：

$$
Z(w)=\sum_{\tau}\exp(R_w(\tau)) \tag{10.40}$$
...
p_w(\tau)
=
\frac{1}{Z(w)}\exp(R_w(\tau)) \tag{10.41}$$
...
$$
Z(w)=\sum_{\tau}\exp(R_w(\tau)) \tag{10.42}$$

它的作用是让概率加起来等于 1：

$$
\sum_\tau p_w(\tau)=1 \tag{10.43}$$
...
$$
Z(w)=\sum_\tau \exp(R_w(\tau)) \tag{10.45}$$
...
p_w(\tau)
=
\frac{1}{Z(w)}\exp(R_w(\tau)) \tag{10.46}$$
...
$$
\mathcal D_E=\{\tau_i^E\}_{i=1}^{N} \tag{10.47}$$
...
\log p_w(\tau_i^E)
=
\log \left(\frac{1}{Z(w)}\exp(R_w(\tau_i^E))\right) \tag{10.49}$$
...
\sum_{i=1}^{N} F(\tau_i^E)
-
N\mathbb E_{\tau\sim p_w(\tau)}[F(\tau)] \tag{10.54}$$
...
\mu_w
=
\mathbb E_{\tau\sim p_w(\tau)}[F(\tau)] \tag{10.56}$$
...
w_4 v^2
+
w_5 \mathbb I[\text{near obstacle}] \tag{10.62}$$

这里用 cost 更符合工程习惯。reward 可以理解为负 cost：

$$
r_w(s,a)=-c_w(s,a) \tag{10.63}$$
...
$$
\max_\theta \sum_t \log \pi_\theta(a_t^E|s_t^E) \tag{10.64}$$
...
$$
\mathcal D_E \rightarrow r_\phi(s,a) \tag{10.65}$$
...
$$
\rho_{\pi_\theta}(s,a)\approx \rho_{\pi_E}(s,a) \tag{10.66}$$
...
$$
\mathcal D_E \longrightarrow r_\phi(s,a) \tag{10.70}$$
...
$$
r'(s,a)=\alpha r(s,a),\quad \alpha>0 \tag{10.71}$$
...
$$
R_w(\tau)=\sum_{t=0}^{T}\gamma^t r_w(s_t,a_t) \tag{10.73}$$
...
$$
F(\tau)=\sum_{t=0}^{T}\gamma^t f(s_t,a_t) \tag{10.74}$$
...
\mu(\pi)
=
\mathbb E_{\tau\sim p_\pi(\tau)}[F(\tau)] \tag{10.75}$$
...
p_w(\tau)
=
\frac{1}{Z(w)}\exp(R_w(\tau)) \tag{10.77}$$
...
$$
Z(w)=\sum_\tau \exp(R_w(\tau)) \tag{10.78}$$
...
\mathcal L(w)
=
\sum_{i=1}^{N}R_w(\tau_i^E)-N\log Z(w) \tag{10.79}$$
...
$$
\frac{1}{N}\nabla_w\mathcal L(w)=\mu_E-\mu_w \tag{10.80}$$
