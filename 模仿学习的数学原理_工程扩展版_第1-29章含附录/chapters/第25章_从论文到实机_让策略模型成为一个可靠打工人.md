...
- Russ Tedrake, *Underactuated Robotics*, online textbook. <https://underactuated.mit.edu/>
...
- 读 Underactuated Robotics 中关于 feedback、stability 和 trajectory optimization 的章节，补齐学习策略之外的控制基础。
- 把每篇策略论文的 benchmark 结果拆成数据、控制频率、硬件、任务范围、安全约束五个维度再比较。
- 实机部署前建议建立 gate：离线指标、仿真回放、影子模式、小流量灰度、fallback 和回滚。
