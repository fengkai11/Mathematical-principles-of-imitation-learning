...
这条主线承接第16章留下的问题。第16章比较了 BC、ACT、Diffusion Policy 和 Flow Matching，重点回答“动作如何建模”。本章换一个角度，回答“策略预测动作时，条件信息如何组织”。

更直接地说：

```text
第四篇：动作输出空间怎么建模？
第五篇第17章：动作预测的上下文怎么建模？
第五篇第18章：视觉、语言和动作如何统一进一个策略模型？
```

本章解决的问题是：为什么现代策略网络不再只看当前观测，而要把历史、目标和多模态条件组织成序列。

本章留下的问题是：当视觉、语言和动作都进入同一个模型时，动作到底应该如何表示、解码和执行？这会自然引出下一章 VLA。
...
|---|---|---|---|---|
| 上下文 | $c_t$ | 时刻 $t$ 预测动作时可用的条件信息 | 历史图像、历史动作、语言指令、目标位姿 | 统一单步策略和序列策略 |
| 任务条件 | $g$ | 控制策略行为方向的外部条件 | 目标位置、任务 ID、语言指令、目标回报 | 连接 Transformer Policy 和 VLA |
| return-to-go | $G_t$ | 从当前时刻到轨迹结束的累计回报 | 轨迹质量、成功进展、人工评分 | 引出 Decision Transformer |
| token 序列 | $z_{1:m}$ | 输入 Transformer 的统一序列表示 | 图像 patch、状态、动作、语言 token | 引出多模态策略结构 |
| 动作 token | $q_t$ | 离散化后的动作符号 | 被量化的末端增量或关节命令 | 连接 VLA 和动作 token 模型 |
| 动作块 | $A_t$ | 从当前时刻开始的一段动作 | 未来 $H$ 步控制命令 | 连接第13章 ACT |
| 历史窗口 | $K$ | 策略实际保留的最近历史长度 | 最近 $K$ 帧观测和动作 | 分析长上下文和实时性 |
| 递推状态 | $h_t$ | 对历史信息的压缩记忆 | 任务阶段、接触历史、失败恢复状态 | 连接 SSM / Mamba |

这些对象说明：本章不是简单介绍一个网络结构，而是在扩展策略的条件变量。

第2章中的策略主要写成“当前观测到当前动作”。第17章中的策略则写成“上下文到动作”。这个变化会影响数据构造、模型结构、训练目标、推理方式和部署风险。
...
其中，上下文记为 $c_t$，表示时刻 $t$ 可用的信息。它可以包含当前观测，也可以包含历史轨迹、任务目标、语言指令、期望回报和其他条件。
...
- 观测历史记为 $o_{1:t}$，表示从第 $1$ 步到第 $t$ 步的观测序列。
- 历史动作记为 $a_{1:t-1}$，表示当前动作之前已经执行过的动作序列。
- 任务条件记为 $g$，可以是目标位置、语言指令、期望回报或任务 ID。
...
注意，公式 (17.5) 本身不是 loss。它描述的是模型希望逼近的数据分布。真正的训练目标要根据动作形式写成 MSE、负对数似然或交叉熵。
...
## 4. Decision Transformer：return / state / action 的序列建模
...
其中，状态记为 $s_t$，动作记为 $a_t$，奖励或任务进展信号记为 $r_t$，轨迹长度记为 $T$。
...
其中，折扣因子记为 $\gamma$，取值范围为 $[0,1]$。为了降低理解成本，本章主要使用不折扣形式。
...
### 4.3 推理时 return-to-go 怎么用

训练时，return-to-go 来自数据轨迹。也就是说，轨迹里的奖励 $r_t$ 已经记录下来，所以可以从后往前计算每个时刻的 $G_t$。

推理时情况不同。机器人还没有走到未来，未来奖励也还没有发生，因此不能直接知道真实的 $G_t$。常见做法是先人为设定一个目标回报，记为 $G_1^{\mathrm{target}}$。模型根据这个目标回报、当前状态和历史动作预测动作。

如果环境在每一步之后能返回奖励或进展分数 $r_t$，那么目标回报可以递减更新：

**公式 (17.10)：推理时目标回报更新**

$$G_{t+1}^{\mathrm{target}} = G_t^{\mathrm{target}} - r_t$$

这个公式的读法是：如果当前这一步已经获得了 $r_t$ 的进展，那么剩余目标回报就相应减少。

在很多机器人模仿任务中，奖励并不像强化学习环境那样天然存在。例如机械臂插入治具、工件摆放、抓取纠偏等任务，数据里可能只有成功/失败标签、人工评分、阶段进度或轨迹质量分。因此在这类任务中，return-to-go 更像“轨迹质量条件”，不一定是严格的强化学习回报。

这也是 Decision Transformer 在机器人任务中需要谨慎使用的原因：如果目标回报构造得不好，模型学到的条件就可能和真实任务成功率脱节。

### 4.4 轨迹 token 化

Decision Transformer 会把轨迹整理成类似下面的序列：

**公式 (17.11)：Decision Transformer 输入序列**
...
**公式 (17.12)：Decision Transformer 的动作预测分布**
...
**公式 (17.15)：连续动作下的 Decision Transformer 训练目标**

$$\mathcal{L}_{DT}^{\mathrm{MSE}}(\theta) = \frac{1}{N}\sum_{i=1}^{N}\sum_{t=1}^{T_i}\lVert f_\theta(G_{1:t}^{(i)},s_{1:t}^{(i)},a_{1:t-1}^{(i)}) - a_t^{(i)}\rVert^2$$

这里，Transformer 输出的动作预测函数记为 $f_\theta$，第 $i$ 条轨迹中第 $t$ 步的数据动作记为 $a_t^{(i)}$。

如果动作被离散化为 action token，也可以使用交叉熵目标：

**公式 (17.16)：离散动作 token 下的训练目标**

$$\mathcal{L}_{DT}^{\mathrm{CE}}(\theta) = -\frac{1}{N}\sum_{i=1}^{N}\sum_{t=1}^{T_i}\log p_\theta(a_t^{(i)} \mid G_{1:t}^{(i)},s_{1:t}^{(i)},a_{1:t-1}^{(i)})$$

这两个目标都说明了一件事：Decision Transformer 的训练形式非常接近监督学习。它不是通过 Bellman backup 反复更新价值估计，而是在离线轨迹上做条件动作预测。

### 4.7 命题：Decision Transformer 是扩展上下文上的条件 BC

> **命题 17.1：Decision Transformer 的监督学习视角**
>
> 如果把上下文定义为 $c_t = (G_{1:t},s_{1:t},a_{1:t-1})$，那么 Decision Transformer 的动作预测训练可以看成在扩展上下文上的行为克隆。

**证明思路**：普通 BC 和 Decision Transformer 的共同点是：监督信号都是数据中的动作。区别在于，普通 BC 的条件变量通常是当前状态或当前观测，而 Decision Transformer 的条件变量包含目标回报、历史状态和历史动作。

普通 BC 可以写成在数据上最小化负对数似然：

**公式 (17.17)：普通 BC 的负对数似然目标**

$$\mathcal{L}_{BC}(\theta) = -\mathbb{E}_{(s_t,a_t)\sim \mathcal{D}}[\log \pi_\theta(a_t \mid s_t)]$$

现在定义扩展上下文：

**公式 (17.18)：Decision Transformer 的扩展上下文**

$$c_t^{DT} = (G_{1:t},s_{1:t},a_{1:t-1})$$

把普通 BC 中的条件变量 $s_t$ 替换为扩展上下文 $c_t^{DT}$，就得到：

**公式 (17.19)：扩展上下文上的条件 BC 目标**

$$\mathcal{L}_{DT}(\theta) = -\mathbb{E}_{(c_t^{DT},a_t)\sim \mathcal{D}}[\log p_\theta(a_t \mid c_t^{DT})]$$

这正是 Decision Transformer 的监督学习形式。连续动作时，可以把负对数似然替换成固定方差高斯假设下的 MSE；离散动作 token 时，可以使用交叉熵。
...
常见误解是把 Decision Transformer 理解成“Transformer 版万能强化学习”。更准确的理解是：它把离线轨迹上的决策问题，改写成了带目标条件和历史上下文的动作预测问题。

---

## 5. Transformer Policy：从历史上下文到动作预测

Decision Transformer 是一个非常典型的例子，但它不是序列策略模型的全部。更一般地说，Transformer Policy 指的是用 Transformer 作为策略网络骨架，接收一段上下文，然后输出动作。

一般形式可以写成：

**公式 (17.20)：Transformer Policy 的一般形式**

$$\pi_\theta(a_t \mid z_{1:m})$$

其中，输入 token 序列记为 $z_{1:m}$。它可以包含很多类型的信息。
...
三种常见 Transformer Policy 结构可以这样理解。

| 结构 | 能看什么 | 典型用途 | 主要风险 |
|---|---|---|---|
| Causal Transformer policy | 过去和当前 token，不能看未来 | 自回归动作预测、Decision Transformer | 历史越长延迟越高，错误会进入后续上下文 |
| Encoder-decoder policy | encoder 编码观测、语言和任务条件，decoder 生成动作 | 语言/视觉条件下的动作序列生成 | 结构更复杂，训练和推理链路更长 |
| Cross-attention policy | 动作 query 从视觉、语言或历史 token 中读取信息 | 多模态条件动作头、动作查询式策略 | query 设计和条件 token 质量影响很大 |
...
**公式 (17.22)：动作 token 分布**

$$p_\theta(q_t \mid z_{1:m})$$

其中，离散动作 token 记为 $q_t$。连续动作可以先量化成 token，再由模型预测 token 分布。
...
**公式 (17.24)：缩放点积 attention**

$$\mathrm{Attn}(Q,K,V) = \mathrm{softmax}\left(\frac{QK^\top}{\sqrt{d}}\right)V$$

这里，查询矩阵记为 $Q$，键矩阵记为 $K$，值矩阵记为 $V$，特征维度记为 $d$。
...
Attention 负责让 token 彼此读取信息，但它本身并不会天然知道“哪个 token 更早、哪个 token 更晚、哪个 token 是图像、哪个 token 是动作”。

因此，序列策略通常还需要额外的编码信息。

**公式 (17.27)：token 表示的组成**

$$z_j = e_j^{\mathrm{content}} + e_j^{\mathrm{pos}} + e_j^{\mathrm{type}}$$

这里可以这样理解：

- 内容 embedding 记为 $e_j^{\mathrm{content}}$，表示 token 自身内容，例如图像 patch、状态向量、动作向量或语言 token；
- 位置 embedding 记为 $e_j^{\mathrm{pos}}$，表示位置或时间编码，用来告诉模型顺序；
- 类型 embedding 记为 $e_j^{\mathrm{type}}$，表示 token 类型编码，用来区分图像、状态、动作、语言、目标等模态。

这个细节在机器人策略中非常重要。因为下面两个序列的控制含义完全不同：
...
如果模型不知道时间顺序，就很难正确判断任务阶段。如果模型不知道 token 类型，就可能把动作 token、状态 token 和语言 token 混在一起理解。

---

## 7. 长序列问题：上下文长度、训练部署不一致、实时性
...
**公式 (17.32)：标准 attention 的长度复杂度**

$$\mathcal{C}_{\mathrm{attn}} = O(L^2 d)$$

其中，token 数记为 $L$，特征维度记为 $d$。
...
### 8.1 RNN、Transformer 与 SSM 的位置关系
...
这里，当前输入记为 $x_t$，压缩后的历史状态记为 $h_t$，输出记为 $y_t$。矩阵 $A$、$B$、$C$ 分别控制历史如何保留、当前输入如何写入以及状态如何读出。
...
本节只帮助读者知道：长历史建模不只有 attention 一条路；但本书后续仍以 Transformer / VLA 主线为主。
...
最重要的变化是条件变量变了。回看公式 (17.1)，单步策略主要根据当前观测 $o_t$ 预测动作。回看公式 (17.4)，序列策略把历史观测、历史动作和任务条件都纳入上下文。回看公式 (17.12)，Decision Transformer 进一步把 return-to-go、状态和动作历史组织成动作预测条件。
...
但本章也留下了新的问题：当视觉、语言和动作都进入同一个策略模型时，动作应该如何表示？语言如何约束动作？视觉 token 和动作 token 如何对齐？这些问题会引出下一章 VLA。
...
第五，训练上下文和部署上下文不一致仍然会造成分布偏移。序列策略缓解了单帧信息不足的问题，但没有自动解决闭环错误累积。

第六，SSM / Mamba 适合作为长时域策略建模的补充视角，但不需要在本书中单独扩展成完整方法主线。
...
causal mask 指自回归模型中用于阻止当前预测访问未来 token 的掩码机制，目的是避免训练时偷看未来答案。

### 定义 17.7：位置编码 / 时间编码 / 模态编码

位置编码和时间编码用于告诉模型 token 的顺序，模态编码用于告诉模型 token 的类型，例如图像、语言、状态或动作。

### 定义 17.8：SSM / Mamba 补充视角
