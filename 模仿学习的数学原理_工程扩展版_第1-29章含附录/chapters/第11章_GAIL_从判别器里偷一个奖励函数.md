...
**读者提示**：实际训练里通常拿不到完整的 $\rho_E(s,a)$，只能拿到有限数据集 $\mathcal{D}_E$。所以 $\rho_E$ 是理论分布，$\mathcal{D}_E$ 是它的数据近似。专家数据覆盖不足时，后续 BC 和 GAIL 都会受到影响。
...
**读者提示**：本章用 $(s,a)$ 是理论记号。真实机器人中，判别器常接收 $(o,a)$，其中 $o$ 可以包含图像、本体状态、末端位姿或其他传感器观测。没有完整状态时，也可以把问题理解为“观测—动作样本”的判别。
...
**读者提示**：实际训练中，GAIL 通常不会显式估计完整的 $\rho_E(s,a)$ 和 $\rho_\pi(s,a)$。它用专家样本和当前策略 rollout 样本训练判别器，让判别器间接学习两类 occupancy measure 的差异。
...
**读者提示**：下面为了便于推导，把状态—动作空间写成离散求和。连续状态或连续动作下，可以把求和换成积分，推导思路和最优判别器形式不变。
...
**读者提示**：密度比可以这样理解：如果某个区域专家多、策略少，那么 $\rho_E(s,a)/\rho_\pi(s,a)$ 大，策略应该更常访问这类区域；如果某个区域专家少、策略多，那么这个比值小，策略应该减少访问这类区域。
...
**读者提示**：GAIL 的 reward 不是训练前固定好的任务函数，而是会随着判别器更新而变化。判别器过弱时反馈不清楚；判别器过强时策略也可能难以获得有效学习信号。因此 GAIL 和 GAN 类似，也存在训练稳定性问题。
...
**读者提示**：策略更新通常不能只像监督学习那样对当前样本做普通反向传播。原因是策略动作会影响后续状态，策略变了，未来访问分布 $\rho_\pi(s,a)$ 也会变。因此 GAIL 通常需要 RL 或 policy gradient 类方法，用判别器 reward 通过 rollout 回报来更新策略。
