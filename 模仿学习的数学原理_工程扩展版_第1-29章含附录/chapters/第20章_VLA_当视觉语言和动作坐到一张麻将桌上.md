...
$$
P_\theta(a_t|o_{\le t},a_{<t},g) \tag{20.2}$$
...
$$
\pi_\theta(a_t|I_{\le t},q_{\le t},a_{<t},l) \tag{20.3}$$
...
$$
\pi_\theta(a_{t:t+K-1}|I_{\le t},q_{\le t},a_{<t},l) \tag{20.4}$$

这里的 $K$ 是 action chunk 的长度。第13章讲 ACT 时我们已经知道，一次输出一段动作可以降低高频控制压力，也可以让模型表达更完整的短期意图。

如果动作被离散化成 token，则可以写成：

$$
P_\theta(y_{t,1:M}|I_{\le t},q_{\le t},y_{<t},l) \tag{20.5}$$
...
$$
c_t=(I_{\le t},q_{\le t},a_{<t},l) \tag{20.6}$$

于是策略可以简写为：

$$
\pi_\theta(a_t|c_t) \tag{20.7}$$
...
$$
a_t = (\Delta x_t,\Delta y_t,\Delta z_t,\Delta r_t,\Delta p_t,\Delta y_t, g_t) \tag{20.8}$$

或者关节空间动作：

$$
a_t=(\dot q_{1,t},\dot q_{2,t},\dots,\dot q_{n,t},g_t) \tag{20.9}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},l) \tag{20.11}$$
...
$$
\pi_\theta(a_t|o_t)
\quad \text{可能是多峰的} \tag{20.12}$$

加入语言后：

$$
\pi_\theta(a_t|o_t,l=\text{``拿红杯子''}) \tag{20.13}$$

$$
\pi_\theta(a_t|o_t,l=\text{``推盘子''}) \tag{20.14}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},g) \tag{20.15}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},l) \tag{20.17}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},\phi(g)) \tag{20.19}$$
...
$$
a_t \in \mathbb R^d \tag{20.20}$$

例如末端位姿增量加夹爪命令：

$$
a_t=(\Delta x_t,\Delta y_t,\Delta z_t,\Delta \mathrm{roll}_t,\Delta \mathrm{pitch}_t,\Delta \mathrm{yaw}_t,g_t) \tag{20.21}$$
...
$$
a_{t,j}\in [m_j,M_j] \tag{20.22}$$

我们把这个区间分成 $B$ 个 bin。量化函数记为：

$$
y_{t,j}=Q_j(a_{t,j})\in \{1,2,\dots,B\} \tag{20.23}$$
...
$$
y_t=(y_{t,1},y_{t,2},\dots,y_{t,d}) \tag{20.24}$$

如果一次生成 $K$ 步动作，那就是：

$$
y_{t:t+K-1}
= (y_t,y_{t+1},\dots,y_{t+K-1}) \tag{20.25}$$

这时动作预测可以变成分类问题：

$$
P_\theta(y_{t,j}|c_t) \tag{20.26}$$
...
$$
\Delta_j=\frac{M_j-m_j}{B} \tag{20.29}$$
...
$$
y_{1:M}=(y_1,y_2,\dots,y_M) \tag{20.31}$$
...
$$
h_t=f_\theta(I_{\le t},q_{\le t},a_{<t},l) \tag{20.34}$$

连续动作头输出：

$$
\hat a_t = W h_t + b \tag{20.35}$$

或者更一般地：

$$
\hat a_t = g_\psi(h_t) \tag{20.36}$$
...
$$
\pi_\theta(a_{t:t+K-1}|c_t) \tag{20.38}$$

或者输出 action token 序列：

$$
P_\theta(y_{t:t+K-1}|c_t) \tag{20.39}$$
...
$$
e_k = \mathrm{Embed}(\mathrm{robot\ type}=k) \tag{20.46}$$

然后策略写成：

$$
\pi_\theta(a_t|c_t,e_k) \tag{20.47}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},l,e) \tag{20.48}$$
...
\quad
\text{for many tasks and embodiments} \tag{20.50}$$
...
\not\subset
\mathrm{support}(\mathcal D_{\mathrm{train}}) \tag{20.52}$$
...
$$
\epsilon_\theta(a_t^k,k,c_t) \tag{20.54}$$

这里 $c_t$ 是 VLA 上下文，$a_t^k$ 是加噪后的动作或动作块，$k$ 是扩散步。这个形式把第14章 Diffusion Policy 的动作生成能力和 VLA 的视觉语言条件结合起来。
...
$$
h_t=f_\theta(I_{\le t},l,q_{\le t},a_{<t}) \tag{20.55}$$

$$
\hat a_t=g_\psi(h_t) \tag{20.56}$$
...
=
\prod_{m=1}^{M}
P_\theta(y_m|x^{I}_{1:N},x^l_{1:L},x^q_{1:R},y_{<m}) \tag{20.57}$$
...
$$
c_t=(I_{t-H:t}^{\mathrm{top}}, I_{t-H:t}^{\mathrm{wrist}}, q_{t-H:t}, a_{t-H:t-1}, l) \tag{20.59}$$

策略：

$$
\pi_\theta(a_t|c_t) \tag{20.60}$$
...
$$
l=\text{``pick up an object that can be used as a hammer''} \tag{20.61}$$

策略：

$$
\pi_\theta(a_t|I_t,q_t,l) \tag{20.62}$$
...
$$
\tilde a_t = \mathrm{clip}(a_t,a_{\min},a_{\max}) \tag{20.64}$$
...
\bar a_t
=
\lambda \bar a_{t-1}+(1-\lambda)\tilde a_t \tag{20.65}$$
...
$$
\mathrm{Safe}(s_t,a_t)=1 \tag{20.66}$$
...
=
-
\sum_y P_\theta(y|c_t)\log P_\theta(y|c_t) \tag{20.67}$$
...
$$
\pi_\theta(a_t|I_{\le t},q_{\le t},a_{<t},l) \tag{20.68}$$
...
$$
\pi_\theta(a_{t:t+K-1}|I_{\le t},q_{\le t},a_{<t},l) \tag{20.69}$$
...
$$
P_\theta(y_{t,1:M}|I_{\le t},q_{\le t},y_{<t},l) \tag{20.70}$$
...
$$
c_t=(I_{\le t},q_{\le t},a_{<t},l) \tag{20.71}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},l) \tag{20.74}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},g) \tag{20.75}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},\phi(g)) \tag{20.76}$$
...
$$
y_{t,j}=Q_j(a_{t,j})\in \{1,2,\dots,B\} \tag{20.77}$$
...
$$
y_t=(y_{t,1},y_{t,2},\dots,y_{t,d}) \tag{20.78}$$
...
$$
y_{t:t+K-1}= (y_t,y_{t+1},\dots,y_{t+K-1}) \tag{20.79}$$
...
$$
\Delta_j=\frac{M_j-m_j}{B} \tag{20.81}$$
...
$$
h_t=f_\theta(I_{\le t},q_{\le t},a_{<t},l) \tag{20.85}$$
...
$$
e_k = \mathrm{Embed}(\mathrm{robot\ type}=k) \tag{20.93}$$
...
$$
\pi_\theta(a_t|o_{\le t},a_{<t},l,e) \tag{20.95}$$
...
d^{\pi_\theta}_{\mathrm{deploy}}(s,o,l)
\not\subset
\mathrm{support}(\mathcal D_{\mathrm{train}}) \tag{20.96}$$
...
$$
\epsilon_\theta(a_t^k,k,c_t) \tag{20.97}$$
...
$$
\tilde a_t = \mathrm{clip}(a_t,a_{\min},a_{\max}) \tag{20.98}$$
...
\bar a_t
=
\lambda \bar a_{t-1}+(1-\lambda)\tilde a_t \tag{20.99}$$
...
$$
\mathrm{Safe}(s_t,a_t)=1 \tag{20.100}$$
...
=
-
\sum_y P_\theta(y|c_t)\log P_\theta(y|c_t) \tag{20.101}$$
